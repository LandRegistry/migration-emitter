...
class DataExtractor
  ...
  def self.get_alias_names(name_hash)
    ...
    if name_hash['alias_names'].present?   #is anything in alias names?
      name_hash['alias_names'].each do |alias_name|   #if alias names present loop through each one
        ...
        proprietor_alias['first_name']    = alias_name['forename'] ? alias_name['forename'] : ''
        proprietor_alias['last_name']     = alias_name['surname'] ? alias_name['surname'] : ''
        ...
        if infill['date'].present?
          ...
          case type
            ...
            when 'address'
              ...
              fields['addresses'].push( populate_address( infill['address'] ) )
            else
              if fields[type].nil?
                fields[type] = []
              end
  ...
  def self.populate_property_schedule_address( address )
    json_address = {}
    address_fields = %w(house_no street_name town postal_county region_name country postcode local_name dx_number leading_information sub_building_description trailing_information sub_building_number house_alpha plot_no secondary_house_alpha plot_code secondary_house_number email_address address_type dx_exchange_name house_description)
    json_address['full_address'] = address
