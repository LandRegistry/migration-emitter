...
class ScheduleExtractor
  ...
  def self.get_d_schedule( model )
    schedule_array = []

    if model['entries'].present?
      model['entries'].each do |entry|
        if entry['role_code'] == 'DIRC'
          ...
          schedule_array.push(schedule_entry) if schedule_entry
        end
      end
    end
  ...
  def self.get_h_schedule( model )
    schedule_entry = {}
    count = 0
    if model['entries'].present?
      model['entries'].each do |entry|
        if entry['role_code'] == 'RSLP'
          if count > 0
            raise 'more than 1 H schedule found'
          else
            count += 1
          end
  ...
  def self.get_schedules( model , schedule_code )
